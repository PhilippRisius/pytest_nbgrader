...
.. warning::
    ...
    It is important to be aware that any changes to files found within the ``src/pytest_nbgrader`` folder (with the exception of ``src/pytest_nbgrader/__init__.py``) will trigger the ``bump-version.yml`` workflow. Be careful not to commit changes to files in this folder when preparing a new release.
...
From the pytest nbgrader plugin source folder we can enter into the docker container, providing access to the `src/pytest_nbgrader` source files by linking them to the running image:
    ...
    .. code-block:: console
        ...
        sudo docker run --rm -ti -v $(pwd):/src/pytest_nbgrader -w /src/pytest_nbgrader quay.io/pypa/manylinux_2_24_x86_64 bash
